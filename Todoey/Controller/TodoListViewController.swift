...
class TodoListViewController: SwipeTableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let hexColor = selectedCategory?.color {
            ...
            guard let uiColor = UIColor(hexString: hexColor) else { fatalError("Color does not exist.") }
            ...
            searchBar.searchFieldBackgroundImage(for: .application)
...
extension TodoListViewController: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        todoItems = todoItems?.filter("title CONTAINS[cd] %@", searchBar.text!).sorted(byKeyPath: "creatingDate", ascending: true)
